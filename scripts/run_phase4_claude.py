"""Phase 4b (Claude-simulated): Description variant selection experiment.

Simulates Claude's tool selection behavior using GPT-4o-mini with a
Claude-specific system prompt. For each of 17 clusters x 3 queries x
5 description variants, test tool selection 3 times. Record hit/miss.

Since we cannot call the Anthropic API directly (OAuth token limitation),
this uses GPT-4o-mini with a system prompt instructing it to behave as
Claude would when selecting tools. Results are labeled accordingly.

Usage:
    uv run python scripts/run_phase4_claude.py
"""
...
RESULTS_PATH = PROJECT_ROOT / "data" / "experiments" / "phase4_claude_results.json"

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------
MODEL = "gpt-4o-mini"
MODEL_LABEL = "claude-simulated-via-gpt-4o-mini"
REPS = 3
VARIANTS = ("V_orig", "V_prose", "V_md", "V_xml", "V_spec")

CLAUDE_SYSTEM_PROMPT = (
    "You are Claude, an AI assistant made by Anthropic. You are acting as a "
    "tool router. Given a user query, you must select the SINGLE most "
    "appropriate tool by calling it. "
    "When choosing between tools with similar names, prioritize:\n"
    "1. Semantic match between the query intent and the tool description\n"
    "2. Specificity of the description — prefer tools whose descriptions "
    "clearly explain WHAT they do and WHEN to use them\n"
    "3. Domain relevance — match the query domain to the tool domain\n"
    "Think carefully about which tool best serves the user's actual need."
)
...
def call_model(
    ...
    """Call GPT-4o-mini with Claude system prompt and tool_choice=required.

    Returns the function name chosen.
    """
    try:
...
def run_experiment() -> dict[str, Any]:
    ...
    # Only process clusters present in all three input files
    valid_ids = set(clusters.keys()) & set(descriptions.keys()) & set(queries.keys())
        ...
        for qdata in cluster_queries:
            for variant in VARIANTS:
                ...
                for _rep in range(REPS):
                    chosen_fn = call_model(client, qdata.query, tools_payload)
    ...
    # ------------------------------------------------------------------
    # Aggregate summary by cluster
    # ------------------------------------------------------------------
    summary_by_cluster: dict[str, dict[str, Any]] = {}
    for cid in cluster_ids:
        c_results = [r for r in results if r.cluster_id == cid]
        total_hits = sum(r.hits for r in c_results)
        total_trials = sum(r.trials for r in c_results)
        hit_rates = [r.hit_rate for r in c_results if r.trials > 0]
        avg_hit_rate = sum(hit_rates) / len(hit_rates) if hit_rates else 0.0
        summary_by_cluster[cid] = {
            "total_hits": total_hits,
            "total_trials": total_trials,
            "avg_hit_rate": round(avg_hit_rate, 4),
        }
    ...
    output = {
        ...
        "total_conditions": len(cluster_ids) * 3 * len(VARIANTS),
...
def main() -> None:
    logger.info(
        f"Phase 4b (Claude-simulated) — {MODEL_LABEL} "
        f"(underlying: {MODEL}), {REPS} reps per condition"
    )
    ...
    logger.info("=== Summary by Cluster ===")
    for cid, stats in output["summary_by_cluster"].items():
        logger.info(
            f"  {cid:14s}: hits={stats['total_hits']:3d}/{stats['total_trials']:3d} "
            f"avg_hit_rate={stats['avg_hit_rate']:.4f}"
        )
