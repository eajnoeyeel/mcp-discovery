...
### Font Family (aspirational, canonical Framer reference)
...
### Font Family — current implementation (free substitutes)

The aspirational list above pins Framer's licensed faces (GT Walsheim, Azeret Mono, Open Runde). Until the team commits to commercial licensing, the shipped bundle uses the following free substitutes loaded from public CDNs in `src/index.css`:

| Role | Aspirational | **Shipped now** | Source |
|------|--------------|-----------------|--------|
| Display | GT Walsheim Framer Medium | **Satoshi** (weight 500) | `api.fontshare.com` — free commercial license |
| Body/UI | Inter Variable | **Inter** | Google Fonts — open source |
| Monospace | Azeret Mono | **JetBrains Mono** | Google Fonts — Apache 2.0 |
| Rounded | Open Runde | *(not shipped)* | — |
| Accent | Mona Sans | *(not shipped)* | — |

**Guidance for contributors:**
- Apply the weights, sizes, line-heights, and OpenType features from the Hierarchy table below. The *numbers* are canonical; the *font families* are the shipped substitutes.
- Satoshi approximates GT Walsheim's geometric compression well enough for the 500-weight display use-case; keep the extreme negative letter-spacing (-5.5px at 110px, etc.) — that gesture carries more of the personality than the typeface itself.
- Do not introduce additional font families without updating this section and `src/index.css` in the same change.
- If the team later licenses GT Walsheim / Azeret Mono / Open Runde, swap the CDN `@import` lines in `src/index.css` and update this table — no component changes should be required because the aspirational names already live in `tailwind.config.ts`.
